document.addEventListener("DOMContentLoaded", () => {
  ...
  // Eliminado el formulario global y el select de actividades
  ...
  async function fetchActivities() {
    try {
      ...
      document.querySelectorAll(".activity-signup-form").forEach((form) => {
        form.addEventListener("submit", handleActivitySignup);
      });
    } catch (error) {
      activitiesList.innerHTML =
  ...
  // Nuevo handler para registro individual por actividad
  async function handleActivitySignup(event) {
    event.preventDefault();
    const form = event.target;
    const activityCard = form.closest(".activity-card");
    const name = activityCard.querySelector("h4").textContent;
